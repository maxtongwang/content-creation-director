...
`director/` 管**创作**（定位 → 选题 → 脚本 → 剪辑）。
`automation/` 管**发布之后的事**（分发 → 记账 → 回采 → 配额 → 排期）。

两层通过 `ledger` 连接：

```
director 产出内容  →  发布  →  ledger 记账  →  数据回采
                                    ↓
                        配额判断 ← 回流到 director/workflows/7-review
```
...
`director/workflows/7-review.md` 的诊断依赖本层的 `ledger` 和数据回采。
**没有 ledger，配额类规则全是空转。**
...
本层的字段名必须与 `director` 的 `templates/profile.template.yaml` 保持一致。

**director 的工作流更新时，本层可能需要跟着改。**
