...
**自动触发是概率性的，不是保证。**三个层次，从最可靠往下：

| 方式 | 怎么做 | 可靠度 |
|---|---|---|
| **显式点名** | Claude Code 里 `/content-creation-director`；或直接说「用编导 skill」 | 确定 |
| 说得具体一点 | 「帮我想几个小红书选题」比「给点内容建议」触发率高得多 | 高 |
| 自动触发 | 描述里的触发词匹配上就会进 | 看你怎么问 |
...
**看不到这一行，就是没触发**——这时候点名再问一次。

**想让它总是生效**：在项目的 `CLAUDE.md` 或 `AGENTS.md` 里加一句

```markdown
做自媒体相关的事（定位、选题、脚本、剪辑、复盘）时，
一律先用 content-creation-director skill，不要凭直觉给建议。
```

中英文都能触发，但**说得越具体越稳**——
「这条选题行不行」「帮我写个脚本」这类明确的话几乎必中。
